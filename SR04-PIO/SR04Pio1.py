...
"""SR04 ultrasonic distance sensor using PIO on Raspberry Pi Pico
    write to output Fifo to trigger"""
...
def _SR04_PIO():
    ...
    mov(y,osr).side(1) [7]  #use trigger as tieout value :-)
    set(x,0)[7]             #x register as echo count
    jmp(y_dec,'wait') .side(0)     #decremet y so no longer 0
    label('wait')
    ...
    label('skip')
    jmp(pin,'echo')         #pin sitll high so loop again
...
class SR04(object):
    def __init__(self,trigger=1,echo=0,sm=0):
        ...
        self.sm.active(1)
        self.sm.put(0)   #triger initial ping    

    def get(self):
            result=self.sm.get()
            self.sm.put(0)   #triger next ping
